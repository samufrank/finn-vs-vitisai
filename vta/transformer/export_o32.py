#!/usr/bin/env python3
"""Export compiled VTA INT4-o8 modules + all parameters — o32 variant.

Same as export_vta_transformer.py but with larger per-module o_tile values
chosen to reduce VTA call count (fc1 in particular drops from 8 M-chunks
to 4). All o×n products stay <= 200 (within the empirical safe zone for
this PL). Output goes to transformer_export_o32/ — the canonical
transformer_export/ from the o16 build is left untouched as a fallback.

Reads ``vta_transformer_deployment_config.json`` (produced by Phase 1-3) and
produces a self-contained ``transformer_export_o32/`` directory with:
